...
#define INITIAL_HASH_SZ     2053 // initial size of hash table
#define MAX_LOAD_PERCENT    65   // maximal load percent

struct session {
    uint32_t laddr, raddr; //local address, remote address
    uint16_t lport, rport; //local port, remote port
    struct timeval tv; // time values
    struct session *next; // next session 
};

struct hash {
    struct session *sessions; // contain a session
    unsigned long sz, count; // size and count
};
         ...
         struct timeval *result)
{
    ...
    for (session = hash->sessions + port; session->next; session = session->next)
        // find requests and corresponding responses
		if (
...
int hash_exist(struct hash *hash, uint32_t laddr, uint32_t raddr, uint16_t lport, uint16_t rport)
{
    struct session *session;
    unsigned long port;
    ...
    port = hash_fun(laddr, raddr, lport, rport) % hash->sz;
    for (session = hash->sessions + port; session->next; session = session->next){
		if (session->next->raddr == raddr && session->next->laddr == laddr && session->next->rport == rport && session->next->lport == lport)
			return 1; 
	}  
    return 0;
}
...
static unsigned long hash_fun(uint32_t laddr, uint32_t raddr, uint16_t lport, uint16_t rport) {
    unsigned long ret;
